...
if (isset($data['userName'])) {
    // echo json_encode(['status' => 'success', 'message' => 'Message received: ' . $data['userName']]);
}
...
$registration = new Registration($data['userName'], $data['password'],  $data['email']);


$myfile = fopen("newfile.txt", "w") or die("Unable to open file!");
$txt = ($registration->errors);
$txt2 = ($registration->messages);
fwrite($myfile, implode($txt));
fwrite($myfile, implode($txt2));
fclose($myfile);

echo json_encode(['status' => 'error', 'message' => implode($txt)]);
